...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['vehicle_id'])) {
    ...
    if ($decision === "buy") {
        mysqli_query($link,
            "UPDATE order_detail 
             SET buy_status = 1 
             WHERE order_id = $order_id AND vehicle_id = $vehicle_id"
        );
    } elseif ($decision === "no_buy") {
        mysqli_query($link,
            "UPDATE order_detail 
             SET buy_status = 0 
             WHERE order_id = $order_id AND vehicle_id = $vehicle_id"
...
$sqlOrder = "
    SELECT 
        o.order_id, o.status, o.test_drive_date, o.test_drive_time,
...
if (!$orderInfo || mysqli_num_rows($orderInfo) == 0) die("Order not found!");
$first = mysqli_fetch_assoc($orderInfo);
/* =============================================================== */

/* ===================== ORDER VEHICLES ========================== */
$sqlDetail = "
    SELECT od.vehicle_id, od.quantity, od.amount, od.created_at, od.buy_status,
           v.model, v.image_url
...
<div class="card-body px-4">
    ...
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <?php
            } elseif ($first['status'] == 4 || $first['status'] == 5) {
                if ($item['buy_status'] == 1) {
                    echo '<span class="text-success fw-bold">Accepted</span>';
                } elseif ($item['buy_status'] == 0) {
                    echo '<span class="text-danger fw-bold">Refused</span>';
                } else {
